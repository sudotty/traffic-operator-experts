...
> **定位（v5.2 交互式证据驱动进化版）**：唯一入口 = `references/EXECUTION-CORE.md`（概念树 + 9 步流程 + 代码映射 + 自进化闭环 + 铁律速查）。本 SKILL 只保留入口与速查，细节全部进字典文件（按需查，概念只讲一次）。
> **能力**：3 把尖刀——①学术级 GEO/AI 深度（可溯源可校正可复算）②可复算决策（公式代替手感 + 军事战略法则层）③专业决策者交付（决策损耗最低）。附属工具（网感标题/公众号）默认不启用。
> **原则**：优点做到极致强，不追求均衡；合规 > 真实 > 用户目标 > 网感风格。

## 使用方式（先看 EXECUTION-CORE，再按需查字典）

1. **全轨任务**：走 `EXECUTION-CORE.md` ② 的 9 步流程（定目标→判状态→看情报→选战场→定打法→执行→验证→复盘→进化）；失败走 REPAIR，任务留 TRACE。
2. **查概念**：看 `EXECUTION-CORE.md` ① 概念树（L0 合规 / L1 原理 / L2 战略法则 / L3 执行循环 / L4 交付 / L5 进化），对应层进字典文件。
3. **引数据**：按铁律速查（见下）+ `live-data-protocol.md` 取数顺序。
...
| L5 | `references/attribution-rhythm.md` / `account-profile-schema.md` / `evals.md` / `rsi-protocol.md` / `rsi-foundations.md` | 进化：AAR 复盘 / 账号基线记忆 / 回归测试（judge+held-out）/ **RSI 自适应自进化 v5.2（三层递归 + 交互模式：定时自主、主动提问，人做闸门 + 五环自适应 + 三 Cell）** / **RSI 第一性原理（六原子 / VSV / 七方法论）** |
...
9. **RSI 自适应自进化（v5.2）**：执行过程按「在场度×风险」切换自主/提问（定时自主+例外上报；主动会话在目标/方案/发布三处用选项卡片提问，预算 ≤3 个）；能力升级人做闸门；每次产出/变更后自评估（evals 回归）；不过走 REPAIR（最小改动，禁改 SKILL 本体）；有效才升级；**升级后必跑全量回归**（改进的改进也要被评估）。

## 一句话总纲

> **零点击时代（68%），内容获客 = 让 AI 引用你**：先看清（情报）→ 打对仗（间接路线）→ 快循环（OODA，Orient=上下文）→ 讲清楚（信号卡）→ 每战复盘（AAR）→ 复盘进技能（进化）。
