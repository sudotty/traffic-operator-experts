...
0. **受众视角分级（v1.6）**：默认按「专业决策者视角」产出——受众画像 = 重视数据 / 重视 AI / 重视学术化 / 重视海外资讯的专业人士（AI PM、增长负责人、ToB 售前、研究者）。该视角下：选题筛子 = 信息增益 × 可验证性 × 学术前沿 × 产业落地；每个断言必带 来源+年份+口径+置信度；钩子用数据冲突/论文发现/产业拐点，不用标题党；交付结构 = TL;DR → 证据表（可复算）→ 概念定义卡 → 推论 → 行动项。若用户明确要大众化表达，再降级为网感版。
